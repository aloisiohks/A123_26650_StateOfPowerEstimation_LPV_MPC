...
function [linMatrices, xhat_plus, ekfData,vhat] = iterEKF_Ts(voltage,Tsk, uk,Tfk, xhat_plus, model,ekfData)
...
%     tic
   ...
   [linMatrices] = linMat_Ts(xhat_plus,model,uk_1,deltaT);
...
%         cellConfig,ekfData,variables);
    ...
    ekfData.irc_bounds = 3*sqrt(SigmaX(2,2));
    ekfData.h_bounds = 3*sqrt(SigmaX(3,3));
%     toc
%     disp('iterEKF')
end
